...
namespace Web
{
    ...
    public class TaxCalculationController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult GetPIRHeaderChargeForEdit(int Id, string HeaderTable, string LineTable)//PurchaseOrderHeader Id
        {
            ...
            var temp = new PurchaseOrderHeaderChargeService(_unitOfWork).GetCalculationFooterListSProc(Id, HeaderTable, LineTable).ToList();
            ViewBag.ChargeType = TransactionDocCategoryConstants.PurchaseInvoiceReturn;
            return PartialView("FooterChargeEdit", temp);
        }
        ...
        [HttpPost]
        public ActionResult PostPIRCalculationFields(List<HeaderChargeViewModel> temp)
        {
            foreach (var item in temp)
            {
                var header = new PurchaseInvoiceReturnHeaderChargeService(_unitOfWork).Find(item.Id);
                header.Rate = item.Rate;
                header.Amount = item.Amount;
                new PurchaseInvoiceReturnHeaderChargeService(_unitOfWork).Update(header);
            }
        ...
        public ActionResult GetPOAHeaderChargeForEdit(int Id, string HeaderTable, string LineTable)//PurchaseOrderHeader Id
        {
        ...
        [HttpPost]
        public ActionResult PostPOACalculationFields(List<HeaderChargeViewModel> temp)
        {
            foreach (var item in temp)
            {
                var header = new PurchaseOrderHeaderChargeService(_unitOfWork).Find(item.Id);
        ...
        public ActionResult GetPQHeaderChargeForEdit(int Id, string HeaderTable, string LineTable)//PurchaseQuotationHeader Id
        {

            var temp = new PurchaseQuotationHeaderChargeService(_unitOfWork).GetCalculationFooterListSProc(Id, HeaderTable, LineTable).ToList();
            ViewBag.ChargeType = TransactionDocCategoryConstants.PurchaseQuotation;
            return PartialView("FooterChargeEdit", temp);
        }
        ...
        [HttpPost]
        public ActionResult PostPQCalculationFields(List<HeaderChargeViewModel> temp)
        {
            foreach (var item in temp)
            {
                var header = new PurchaseQuotationHeaderChargeService(_unitOfWork).Find(item.Id);
                header.Rate = item.Rate;
                header.Amount = item.Amount;
                new PurchaseQuotationHeaderChargeService(_unitOfWork).Update(header);
            }
