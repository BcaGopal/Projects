...
namespace Web
{
    ...
    public class ComboHelpListController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult GetGate(string searchTerm, int pageSize, int pageNum)
        {
            //Get the paged results and the total count of the results for this query. ProductCacheKeyHint
            var productCacheKeyHint = "GateCacheKeyHint";

            //THis statement has been changed because GetProductHelpList was calling again and again. 

            AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(cbl.GetGateHelpList(), productCacheKeyHint, RefreshData.RefreshProductData);
            //AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(null, productCacheKeyHint);
        ...
        public JsonResult SetGate(string Ids)
        {
            string[] subStr = Ids.Split(',');
            List<ComboBoxResult> ProductJson = new List<ComboBoxResult>();
            for (int i = 0; i < subStr.Length; i++)
            {
                int temp = Convert.ToInt32(subStr[i]);
                //IEnumerable<Product> products = db.Products.Take(3);
                IEnumerable<Gate> prod = from p in db.Gate
                                         where p.GateId == temp
                                         select p;
                ProductJson.Add(new ComboBoxResult()
                {
                    id = prod.FirstOrDefault().GateId.ToString(),
                    text = prod.FirstOrDefault().GateName
                });
            }
            return Json(ProductJson);
        }

        public JsonResult SetSingleGate(int Ids)
        {
            ComboBoxResult ProductJson = new ComboBoxResult();
            ...
            ProductJson.id = prod.FirstOrDefault().GateId.ToString();
            ProductJson.text = prod.FirstOrDefault().GateName;
        ...
        public ActionResult GetProductCategory(string searchTerm, int pageSize, int pageNum, int? filter)
        {
            //Get the paged results and the total count of the results for this query. ProductCacheKeyHint
            var productCacheKeyHint = ConfigurationManager.AppSettings["ProductGroupCacheKeyHint"];
            ...
            AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(cbl.GetProductCategoryHelpList(filter), productCacheKeyHint, RefreshData.RefreshProductData);
            //AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(null, productCacheKeyHint);
        ...
        public JsonResult GetCustomProducts(string searchTerm)//SupplierID
        {
            return Json(cbl.GetProductsHelpList(searchTerm), JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetPersonRateGroupSiteFilter(string searchTerm, int pageSize, int pageNum, int filter)//filter:PersonId
        {
            var temp = cbl.GetPersonRateGroupHelpList(searchTerm, filter).Skip(pageSize * (pageNum - 1)).Take(pageSize).ToList();

            var count = cbl.GetPersonRateGroupHelpList(searchTerm, filter).Count();
        ...
        public JsonResult SetPersonRateGroup(string Ids)
        {
            string[] subStr = Ids.Split(',');
            List<ComboBoxResult> ProductJson = new List<ComboBoxResult>();
            for (int i = 0; i < subStr.Length; i++)
            {
                int temp = Convert.ToInt32(subStr[i]);
                IEnumerable<ComboBoxResult> prod = (from b in db.PersonRateGroup
                                                    where b.PersonRateGroupId == temp
                                                    select new ComboBoxResult
                                                    {
                                                        id = b.PersonRateGroupId.ToString(),
                                                        text = b.PersonRateGroupName
                                                    });
                ProductJson.Add(new ComboBoxResult()
                {
        ...
        public JsonResult GetChargeType(string searchTerm, int pageSize, int pageNum)
        {
            var Query = cbl.GetChargeType(searchTerm);
            var temp = Query.Skip(pageSize * (pageNum - 1)).Take(pageSize).ToList();
        ...
        {
            ComboBoxResult ChargeTypeJson = new ComboBoxResult();

            ChargeType ChargeType = (from b in db.ChargeType
                                                     where b.ChargeTypeId == Ids
                                                     select b).FirstOrDefault();

            ChargeTypeJson.id = ChargeType.ChargeTypeId.ToString();
            ChargeTypeJson.text = ChargeType.ChargeTypeName;

            return Json(ChargeTypeJson);
        }
        public JsonResult SetChargeType(string Ids)
        {
            string[] subStr = Ids.Split(',');
            List<ComboBoxResult> ProductJson = new List<ComboBoxResult>();
            for (int i = 0; i < subStr.Length; i++)
            {
                ...
                IEnumerable<ChargeType> prod = from p in db.ChargeType
                                                      where p.ChargeTypeId == temp
                                                      select p;
                ProductJson.Add(new ComboBoxResult()
                {
                    id = prod.FirstOrDefault().ChargeTypeId.ToString(),
                    text = prod.FirstOrDefault().ChargeTypeName
                });
            }
            return Json(ProductJson);
        }
        ...
        public JsonResult GetPerson(string searchTerm, int pageSize, int pageNum)
        {
            var Query = cbl.GetPerson(searchTerm);
            var temp = Query.Skip(pageSize * (pageNum - 1)).Take(pageSize).ToList();
        ...
        public JsonResult SetSinglePerson(int Ids)
        {
            ComboBoxResult PersonJson = new ComboBoxResult();

            Person Person = (from b in db.Persons
                                     where b.PersonID == Ids
                                     select b).FirstOrDefault();

            PersonJson.id = Person.PersonID.ToString();
            PersonJson.text = Person.Name;
