...
namespace Web
{
    ...
    public class ComboHelpListController : System.Web.Mvc.Controller
    {
        ...
        public JsonResult SetCountry(string Ids)
        {
            string[] subStr = Ids.Split(',');
            List<ComboBoxResult> ProductJson = new List<ComboBoxResult>();
            for (int i = 0; i < subStr.Length; i++)
            {
                ...
                //IEnumerable<Product> products = db.Products.Take(3);
                IEnumerable<Country> prod = from p in db.Country
                                             where p.CountryId == temp
                                             select p;
                ProductJson.Add(new ComboBoxResult()
                {
                    id = prod.FirstOrDefault().CountryId.ToString(),
                    text = prod.FirstOrDefault().CountryName
                });
            }
            return Json(ProductJson);
        }
        ...
        public ActionResult GetCalculationProduct(string searchTerm, int pageSize, int pageNum)
        {
            var productCacheKeyHint = ConfigurationManager.AppSettings["CalculationProductCacheKeyHint"];
            AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(cbl.GetCalculationProductHelpList(), productCacheKeyHint, RefreshData.RefreshProductData);
            if (RefreshData.RefreshProductData == true) { RefreshData.RefreshProductData = false; }
        ...
        public JsonResult SetSingleCalculationProduct(int Ids)
        {
            ComboBoxResult ProductJson = new ComboBoxResult();

            var prod = (from p in db.CalculationProduct
                                                    join t in db.Charge on p.ChargeId equals t.ChargeId
                                                    where p.CalculationProductId==Ids
                                                    select new
                                                    {
        ...
        public ActionResult GetCalculationFooter(string searchTerm, int pageSize, int pageNum)
        {
            var productCacheKeyHint = ConfigurationManager.AppSettings["CalculationFooterCacheKeyHint"];
            AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(cbl.GetCalculationFooterHelpList(), productCacheKeyHint, RefreshData.RefreshProductData);
            if (RefreshData.RefreshProductData == true) { RefreshData.RefreshProductData = false; }
        ...
        public JsonResult SetProductNature(string Ids)
          ...
          public JsonResult SetProductManufacturer(string Ids)
          {
              string[] subStr = Ids.Split(',');
              List<ComboBoxResult> ProductJson = new List<ComboBoxResult>();
              for (int i = 0; i < subStr.Length; i++)
          ...
          public JsonResult SetProductProcessSequence(string Ids)
          {
              string[] subStr = Ids.Split(',');
              List<ComboBoxResult> ProductJson = new List<ComboBoxResult>();
              for (int i = 0; i < subStr.Length; i++)
              {
                  ...
                  IEnumerable<ProcessSequenceHeader> prod = from p in db.ProcessSequenceHeader
                                                      where p.ProcessSequenceHeaderId == temp
                                                      select p;
                  ProductJson.Add(new ComboBoxResult()
                  {
                      id = prod.FirstOrDefault().ProcessSequenceHeaderId.ToString(),
                      text = prod.FirstOrDefault().ProcessSequenceHeaderName
                  });
              }
              return Json(ProductJson);
          }
