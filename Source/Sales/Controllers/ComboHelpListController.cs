...
namespace Web
{
    ...
    public class ComboHelpListController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult GetPersonForSale(string searchTerm, int pageSize, int pageNum)
        {
            var productCacheKeyHint = ConfigurationManager.AppSettings["PersonCacheKeyHint"];
            AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(cbl.GetPersonForSaleHelpList(searchTerm), productCacheKeyHint, RefreshData.RefreshProductData);
            if (RefreshData.RefreshProductData == true) { RefreshData.RefreshProductData = false; }
        ...
        public ActionResult GetShipMethods(string searchTerm, int pageSize, int pageNum)
        {
            //Get the paged results and the total count of the results for this query. ProductCacheKeyHint
            var productCacheKeyHint = ConfigurationManager.AppSettings["ShipMethodCacheKeyHint"];

            //THis statement has been changed because GetProductHelpList was calling again and again. 

            AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(cbl.GetShipMethodHelpList(), productCacheKeyHint, RefreshData.RefreshProductData);
            //AutoCompleteComboBoxRepositoryAndHelper ar = new AutoCompleteComboBoxRepositoryAndHelper(null, productCacheKeyHint);
        ...
        public JsonResult SetSingleShipMethod(int Ids)
        {
            ...
            ComboBoxResult ShipMethodJson = (from b in db.ShipMethod
                                      where b.ShipMethodId == Ids
                                      select new ComboBoxResult
                                      {
                                          id = b.ShipMethodId.ToString(),
                                          text = b.ShipMethodName
                                      }).FirstOrDefault();
        ...
        public JsonResult SetSinglePromoCode(int Ids)
        {
            ComboBoxResult ProductJson = new ComboBoxResult();

            IEnumerable<PromoCode> prod = from p in db.PromoCode
                                        where p.PromoCodeId == Ids
                                        select p;

            ProductJson.id = prod.FirstOrDefault().PromoCodeId.ToString();
            ProductJson.text = prod.FirstOrDefault().PromoCodeName;
